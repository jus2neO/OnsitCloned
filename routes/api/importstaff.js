...
router.post('/',fileUpload(uploadOpts), async (req, res, next) => {
    try{
        ...
        for(let i = 0; i < data.length; i++){
            const {FirstName, LastName, MiddleName, EmailAddress, ContactNo} = data[i];

            const mydata = {fname: FirstName,lname: LastName,mname: MiddleName,email: EmailAddress,contact: ContactNo,status: "pending", role: 0, created: new Date()}
