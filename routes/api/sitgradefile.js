...
const Modelclass = require('../models/sitgradefile');
...
// @access  Private
router.get('/getbyarchieveid/:id', async (req, res, next) => {
    try{
        await Modelclass.findAllByCol("studentarchieceid", req.params.id).then((value) => 
        {
...
router.post('/',upload.single('photo'), async (req, res, next) => {
    try{
        const { studentid, studentarchieceid, filename, size, type, file } = req.body;
        const mydata = {studentid: studentid, studentarchieceid: studentarchieceid, filename: filename, size: size, type: type,file: file,created: new Date()}
